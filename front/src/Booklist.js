...
const Booklist = () => {
    ...
    const getBooks = async () => {
        try {
            let result = await fetch("https://cc-ca-0eps.onrender.com/book-list");
    ...
    const requestBook = async (book) => {
        try {
            const response = await fetch('https://cc-ca-0eps.onrender.com/request-book', {
                method: 'POST',
                headers: {
    ...
    const deleteProduct = async (id) => {
        try {
            let result = await fetch(`https://cc-ca-0eps.onrender.com/book/${id}`, {
                method: 'DELETE'
            });
            let data = await result.json();
            if (data) {
